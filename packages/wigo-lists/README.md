# Wigoswap Token Lists

This repo contains main WigoSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to WigoSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to WigoSwap (extended) token list
...
- Add new tokens to `src/tokens/wigoswap-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on WigoSwap v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (wigoswap-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.wigoswap.io/list-name.json`
