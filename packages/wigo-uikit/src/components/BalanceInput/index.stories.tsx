...
export const UnitDisplay: React.FC<React.PropsWithChildren<unknown>> = () => {
  ...
  const wigoToUSD = (input: string) => {
    const convertedToUSD = parseFloat(input) * WIGO_PRICE;
  ...
  const handleWigoChange = (input: string) => {
    setWigoValue(input);
  };
  ...
  return (
    <>
      <Box width="300px" mb="24px">
        <BalanceInput
          onUserInput={handleWigoChange}
          value={wigoValue}
          currencyValue={wigoToUSD(wigoValue)}
          placeholder="0.0"
          unit="WIGO"
        />
      </Box>
...
export const SiwtchUnits: React.FC<React.PropsWithChildren<unknown>> = () => {
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "WIGO" ? "USD" : "WIGO";
  ...
  const handleWigoChange = (input: string) => {
    ...
    if (editingUnit === "WIGO") {
      setValues({
        WIGO: input,
        USD: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat * WIGO_PRICE}`,
      });
    } else {
      setValues({
        WIGO: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / WIGO_PRICE}`,
        USD: input,
      });
    }
  };
